...
import 'package:agrivoice/authenticate/auth_gate.dart';
import 'package:agrivoice/authenticate/auth_service.dart';
import 'package:flutter/material.dart';
import 'package:agrivoice/app_translations.dart';

// ─────────────────────────────────────────────────────────────
//  OFFLINE PROFILE STORE  (runtime-only, resets on app restart)
// ─────────────────────────────────────────────────────────────
...
class _ProfileStore {
  ...
  static bool   loaded   = false; // guard: only load from AuthService once
}
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  // ── Load logged-in user on first mount ───────────────────
  @override
  void initState() {
    super.initState();
    _loadCurrentUser();
  ...
  Future<void> _loadCurrentUser() async {
    // Only pull from AuthService the very first time
    if (_ProfileStore.loaded) return;
    final username = await AuthService.currentUsername();
    if (!mounted) return;
    setState(() {
      _ProfileStore.name   = username ?? '';
      _ProfileStore.loaded = true;
    });
  }
  ...
  Widget _profileCard(BuildContext context, String Function(String) t) =>
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        // ── Shows actual logged-in username ──
        Text(
          _ProfileStore.name.isEmpty ? '...' : _ProfileStore.name,
          style: const TextStyle(
              color: Colors.white, fontSize: 18, fontWeight: FontWeight.w700),
        ),
        const SizedBox(height: 3),
        // Location row — show only if filled
        if (_ProfileStore.location.isNotEmpty) ...[
          Row(children: [
        ...
        // Farm row — show only if filled
        if (_ProfileStore.land.isNotEmpty || _ProfileStore.crops.isNotEmpty)
          Row(children: [
            const Icon(Icons.agriculture_rounded, color: Colors.white70, size: 13),
            const SizedBox(width: 3),
            Flexible(child: Text(
                [_ProfileStore.land, _ProfileStore.crops]
  ...
  //  FIX: messenger captured BEFORE Navigator.pop to avoid
        ...
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(mainAxisSize: MainAxisSize.min,
              ...
              _editField(ctrl: nameCtrl,     label: t('full_name'),     icon: Icons.person_rounded,    hint: 'e.g. Rajesh Kumar'),
              const SizedBox(height: 14),
              _editField(ctrl: phoneCtrl,    label: t('phone_number') , icon: Icons.phone_rounded,     hint: '+91 XXXXX XXXXX', type: TextInputType.phone),
              const SizedBox(height: 14),
              _editField(ctrl: locationCtrl, label: t('location'),      icon: Icons.location_on_rounded,hint: 'e.g. Pune, Maharashtra'),
              const SizedBox(height: 14),
              _editField(ctrl: landCtrl,     label: t('land_size'),     icon: Icons.landscape_rounded,  hint: 'e.g. 4.5 acres'),
              const SizedBox(height: 14),
              _editField(ctrl: cropsCtrl,    label: t('crop_history'),  icon: Icons.grass_rounded,      hint: 'e.g. Wheat, Tomato, Onion'),
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  onPressed: () {
                    ...
                    if (landCtrl.text.trim().isNotEmpty)
                      _ProfileStore.land     = landCtrl.text.trim();
                    if (cropsCtrl.text.trim().isNotEmpty)
                      _ProfileStore.crops    = cropsCtrl.text.trim();

                    // ✅ KEY FIX: capture messenger BEFORE closing the sheet
                    //    so it is no longer a dependent of the bottom sheet's
                    //    context when we call showSnackBar.
              ...
              itemBuilder: (_, i) {
                ...
                return Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  InkWell(
                    ...
                    child: Container(
                      color: sel ? _green.withOpacity(0.06) : Colors.transparent,
                      padding: const EdgeInsets.symmetric(
                          horizontal: 16, vertical: 11),
                      child: Row(children: [
                        Text(li.flag, style: const TextStyle(fontSize: 24)),
                        const SizedBox(width: 14),
                        Expanded(child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            actions: [
              ...
              TextButton(
                onPressed: () async {
                  ...
                  // Reset profile store so next login starts fresh
                  _ProfileStore.name     = '';
                  _ProfileStore.phone    = '';
                  _ProfileStore.location = '';
                  _ProfileStore.land     = '';
                  _ProfileStore.crops    = '';
                  _ProfileStore.loaded   = false;
                  await LogoutNotifier.of(context).logout();
                },
